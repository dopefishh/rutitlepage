...
release: rutitlepage.tlpobj all
	...
	cp rutitlepage.sty *logo* tex/latex/rutitlepage
...
ctan: rutitlepage.ins rutitlepage.dtx rutitlepage.pdf README.md
	ctanify --no-skip $^ $(addsuffix =tex/latex/rutitlepage,$(wildcard *logo*))
...
clean:
	$(RM) $(addprefix rutitlepage.,sty aux log idx glo pdf ind ilg tar.xz tar.gz fls synctex.gz fdb_latexmk)
